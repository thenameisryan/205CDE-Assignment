...
$email = "";
$superuser = "admin";
$superpass = base64_encode("admin");
$errors = array(); 
//$randNum = mt_rand(1, 9999);

// CONNECT TO THE DATABASE
$db = mysqli_connect('localhost', 'root', '', 'ims_hardware');
if (!$db) {
    die('Could not connect: ' . mysql_error());
}

//REDIRECT USER TO HOME IF THEY DONT HAVE PRIVILEGE TO VIEW SPECIFIC PAGE
...
if (isset($_POST['reg_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO user (username, useremail, userpass, user_type, date_created, created_by) 
  			  VALUES('$username', '$email_reg', '$password', 'USER', '$datenow', 'USER')";
  	mysqli_query($db, $query);
  	$user_id_query = "SELECT * FROM user WHERE username='$username' AND useremail='$email_reg'";
...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
	...
	$query = "SELECT * FROM user WHERE (username='$username' OR useremail='$email') AND userpass='$password' LIMIT 1";
